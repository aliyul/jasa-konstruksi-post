...
const urlMappingSewaBekistingFromMoneyMasterMoneyMaster1 = {
 // MONEY PAGE
...
const urlMappingSewaSelangProyekProyekFromMoneyMasterMoneyMaster1 = {
 // MONEY PAGE
...
/**
 * generateBreadcrumbForMapping v7.0 — UNIVERSAL ENTITY HIERARCHY ENGINE
 * 
 * ✅ UPDATE: ENTITY PILLAR disesuaikan dengan struktur FINAL:
 *    - jasa konstruksi
 ...
 *    - material konstruksi
 * 
 * ✅ UPDATE: HANYA 5 ROOT PILLAR VALID
 * ✅ UPDATE: EXACT MATCH CLEAN URL ONLY
 * ✅ UPDATE: AUTO NORMALIZATION ENTITY TYPE
 * ✅ UPDATE: MAX 4 LEVEL tetap dipertahankan
 * ✅ UPDATE: BREADCRUMB LOGIC tetap dipertahankan
 * ✅ UPDATE: MONEY hierarchy tetap sinkron
 * ✅ UPDATE: LOCATION detection tetap dipertahankan
 * ✅ UPDATE: NO OVER-DETECTION pada page komersial
 * 
 * HIERARKI FINAL:
 * HOME
 * └── PILLAR
...
function generateBreadcrumbJasaAlatKonstruksiPost(
    ...
    // ============================================================
    // 8. EXACT ENTITY PILLAR DETECTION
    // ============================================================
    ...
    function isEntityPillarExactMatch(pageName) {
        ...
        const validPillars = ROOT_ENTITY_PILLARS[entityType] || [];

        return validPillars.includes(cleanName);
    }
    ...
    function isLocation(text) {
        ...
        for (const word of words) {
            ...
            if (LOCATION_WHITELIST.has(clean)) {
    ...
    function isSubVariant(text) {
        ...
        if (/mm|cm|meter|m|kg|ton/i.test(text)) score++;

        return score >= 2;
    }

    // ============================================================
    // 12. PAGE TYPE DETECTION
    // ============================================================
        ...
        for (const kw of MONEY_KEYWORDS) {
            ...
            if (lowerName.includes(kw)) {
                ...
                if (isSewaEntity()) {
                    ...
                    const afterSewa = lowerName
                        .split(kw)[1]
                        ?.trim() || '';

                    const count = afterSewa
                        .split(/\s+/)
                        .filter(Boolean)
                        .length;
                ...
                const afterMoney = lowerName
                    .split(kw)[1]
                    ?.trim() || '';

                const wordCount = afterMoney
                    .split(/\s+/)
                    .filter(Boolean)
                    .length;
    ...
    for (const level of allLevels) {
        ...
        if (!level.url) {
            ...
            if (!foundUrl) {
                ...
                foundUrl = `${DOMAIN}/p/${slugify(level.name)}.html`;
            }
    ...
    const nonHomeLevels = allLevels.filter(
        l => l.type !== 'home'
    );
    ...
    if (nonHomeLevels.length > 0) {
        ...
        parentTerdekat =
            nonHomeLevels[nonHomeLevels.length - 1];
...
`<div class="breadcrumbs" itemscope itemtype="https://schema.org/BreadcrumbList">\n`;
    ...
    for (let i = 0; i < selectedLevels.length; i++) {
        ...
        const item = selectedLevels[i];

        const isLast =
            i === selectedLevels.length - 1;
...
<meta itemprop="position" content="${item.position}" />
    ...
    const jsonLd = {
        ...
        "itemListElement": selectedLevels.map((item, index) => ({
 ...
 * harga beton precast           → money-page (L5) ✅
 * sewa excavator                → money-master (L4) ✅
 * harga sewa excavator          → money-page (L5) ✅
 * sewa excavator jakarta        → money-child (L6) ✅
 * 
 * INFORMASIONAL:
 * 
 * jenis produk konstruksi       → sub-pillar-tipe-2 (L2) ✅
 * produk konstruksi vs precast  → sub-pillar-tipe-1 (L3) ✅
 * panduan produk konstruksi     → pillar (L1) ✅
 * 
 * VARIANT:
 * 
 * spesifikasi beton precast     → variant (L7) ✅
 * mutu beton k300               → variant (L7) ✅
 * panel 10x20x30 cm             → sub-variant (L8) ✅
 * 
 * BREADCRUMB:
 * 
 * MAX 4 LEVEL                   → tetap dipertahankan ✅
 * Exact Match Pillar            → tetap dipertahankan ✅
 * Money Hierarchy               → tetap dipertahankan ✅
 * JSON-LD                       → tetap dipertahankan ✅
 * SEO Safe                      → tetap dipertahankan ✅
 */
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const urlMappingGabungan = Object.assign(
      {},
		...
		urlMappingSewaPompaDewateringFromMoneyMasterMoneyPage,
		urlMappingSewaPompaAirFromMoneyMasterMoneyPage,
		urlMappingSewaPompaLumpurFromMoneyMasterMoneyPage,
		urlMappingSewaBekistingFromMoneyMasterMoneyPage,
		urlMappingSewaPencahayaanProyekFromMoneyMasterMoneyPage,
		urlMappingSewaAlatSurveyFromMoneyMasterMoneyPage,
		urlMappingSewaAlatBorFromMoneyMasterMoneyPage,
		urlMappingSewaTangkiAirFromMoneyMasterMoneyPage,
		urlMappingSewaAksesKeamananProyekFromMoneyMasterMoneyPage,
		urlMappingSewaSelangProyekProyekFromMoneyMasterMoneyPage,
		urlMappingSewaPipaProyekFromMoneyMasterMoneyPage,
...
if (urlMappingSewaPompaDewateringFromMoneyMasterMoneyPage[cleanUrlJasaKonsAlatKonstruksiPost]) {
	...
	if (urlMappingSewaPompaAirFromMoneyMasterMoneyPage[cleanUrlJasaKonsAlatKonstruksiPost]) {
		...
		if (urlMappingSewaPompaLumpurFromMoneyMasterMoneyPage[cleanUrlJasaKonsAlatKonstruksiPost]) {
	...
	if (urlMappingSewaBekistingFromMoneyMasterMoneyPage[cleanUrlJasaKonsAlatKonstruksiPost]) {
		...
		if (urlMappingSewaPencahayaanProyekFromMoneyMasterMoneyPage[cleanUrlJasaKonsAlatKonstruksiPost]) {
	...
	if (urlMappingSewaAlatSurveyFromMoneyMasterMoneyPage[cleanUrlJasaKonsAlatKonstruksiPost]) {
        ...
        'SEWA_RENTAL'
			...
			if (urlMappingSewaTangkiAirFromMoneyMasterMoneyPage[cleanUrlJasaKonsAlatKonstruksiPost]) {
	...
	if (urlMappingSewaAksesKeamananProyekFromMoneyMasterMoneyPage[cleanUrlJasaKonsAlatKonstruksiPost]) {
		...
		if (urlMappingSewaSelangProyekProyekFromMoneyMasterMoneyPage[cleanUrlJasaKonsAlatKonstruksiPost]) {
	...
	if (urlMappingSewaPipaProyekFromMoneyMasterMoneyPage[cleanUrlJasaKonsAlatKonstruksiPost]) {
